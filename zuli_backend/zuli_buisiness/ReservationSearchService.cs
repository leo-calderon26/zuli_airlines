...
namespace zuli_Business
{
    public class ReservationSearchService : IReservationSearchService
    {
        ...
        public async Task<ReservationSearchResponseDTO> GetReservationDetailsAsync(ReservationSearchRequestDTO request)
        {
            ...
            if (!flights.Any())
            {
                throw new ZuliNotFoundException("No se encontró ninguna reservación con los datos proporcionados.");
            }

            var firstFlight = flights.First();
            var daysRemaining = (firstFlight.DepartureDateTime.Date - _timeProvider.GetLocalNow().DateTime.Date).Days;
        ...
        private List<ReservationSearchLayoverDTO> CalculateLayovers(List<ReservationSearchFlightEntity> flights)
        {
            ...
            for (int i = 0; i < flights.Count - 1; i++)
            {
                layovers.Add(new ReservationSearchLayoverDTO
                {
                    ...
                    DurationMinutes = (int)(flights[i + 1].DepartureDateTime - flights[i].ArrivalDateTime).TotalMinutes
                });
            }
            return layovers;
        }
