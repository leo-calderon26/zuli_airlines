    ...
    <div class="flex flex-col min-h-screen bg-body">
        ...
        <main class="flex-1 w-full max-w-7xl mx-auto px-4 py-8">
            <div class="bg-primary rounded-xl p-6 text-white mb-8 shadow-md flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    ...
                    <p class="text-white/80 mt-1 font-medium">
                        ...
                        <span v-if="searchStore.searchParams.IsRoundTrip">
                            | Regreso: {{ searchStore.searchParams.ReturnDate || route.query.returnDate }}
                        </span>
            ...
            <div
                ...
                class="bg-error-soft text-error p-6 rounded-xl border border-error-border text-center font-semibold text-lg"
            >
                {{ searchStore.error }}
            </div>
            ...
            <div v-else-if="searchStore.flightResults">
                <div v-if="!selectedDepartureFlight">
                    ...
                    <div
                        v-if="searchStore.flightResults.departureFlights?.length === 0"
                        class="text-center py-12 text-content-muted font-medium"
                    >
                    ...
                    <div
                        v-if="searchStore.flightResults.totalPagesDeparture > 1"
                ...
                <div v-else-if="searchStore.searchParams.IsRoundTrip">
                    <div class="bg-surface-muted border border-border-light rounded-xl p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
                        <div class="flex-1 w-full md:w-auto text-center md:text-left">
                            <p class="text-sm text-content-subtle font-medium">
                                Su viaje a:
                            </p>

                            <h3 class="text-2xl font-bold text-heading mt-1">
                                {{ selectedDepartureFlight.flight.destination }}
                            </h3>
                        </div>
                    ...
                    <div
                        v-if="searchStore.flightResults.returnFlights?.length === 0"
                        class="text-center py-12 text-content-muted font-medium"
                    >
                    ...
                    <div
                        v-if="searchStore.flightResults.totalPagesReturn > 1"
                        class="flex justify-center items-center gap-4 mt-8"
                    >
                        <AppButton 
                            variant="outline"
