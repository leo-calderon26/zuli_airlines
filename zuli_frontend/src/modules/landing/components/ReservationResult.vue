<template>
  <div class="w-full flex flex-col z-20">
    <div class="w-full mb-6 flex justify-start">
      <button @click="$emit('clear')" class="flex items-center gap-2 text-white hover:text-gold transition-colors font-semibold text-lg drop-shadow-md">
    ...
    <div class="bg-white rounded-xl p-8 shadow-2xl mb-8 w-full border border-border-xlight">
      ...
      <h2 class="text-3xl font-bold text-primary mb-3">
        Viaje a {{ reservationData.destinationCity }} <span class="font-semibold text-content-light">{{ reservationData.destinationCode }}</span>
      </h2>
      
      <div class="flex flex-wrap items-center text-sm font-medium text-content-subtle gap-2">
        <span>Reserva: <strong class="text-primary tracking-wider uppercase">{{ reservationData.reservationCode }}</strong></span>
        <span class="text-border-medium">|</span>
        <span>{{ formatDateRange(reservationData.journey.departureDateTime) }}</span>
        <span class="text-border-medium">|</span>
        <span>{{ reservationData.passengerCount }} Pasajero(s)</span>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full">
      <div class="lg:col-span-2 space-y-6">
        ...
        <FlightItinerary :journey="reservationData.journey" :passengers="reservationData.passengers" />
      </div>
      ...
      <div class="space-y-6 lg:mt-11">
        ...
        <div class="bg-white rounded-xl p-6 shadow-2xl">
          <h3 class="font-bold text-content text-lg mb-4 border-b border-border-xlight pb-3">Gestión de la reserva</h3>
          <div class="space-y-3">
            ...
            <button class="group w-full flex justify-between items-center bg-error-soft hover:bg-primary hover:text-white hover:border-transparent transition-all p-4 rounded-lg text-sm font-semibold text-error border border-error-border">
              <span class="flex items-center gap-3">
                <svg class="w-5 h-5 text-error opacity-80 group-hover:text-white group-hover:opacity-100 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                Cancelar reservación
              </span>
              <svg class="w-4 h-4 text-error opacity-60 group-hover:text-white group-hover:opacity-100 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
            </button>
          </div>
        </div>
      </div>
...
const statusLabel = computed(() => {
  const rawStatus = props.reservationData.status || props.reservationData.reservationStatus;
  
  if (rawStatus) {
    const s = String(rawStatus).toLowerCase().trim();
    if (s === 'active' || s === 'activa' || s === 'confirmada' || s === 'confirmed') return 'Activa';
    if (s === 'cancelled' || s === 'cancelada' || s === 'cancelado') return 'Cancelada';
    if (s === 'concluded' || s === 'completed' || s === 'concluida' || s === 'concluido' || s === 'finalizado') return 'Concluida';
    return rawStatus;
  }
  
  const arrivalDate = new Date(props.reservationData.journey?.arrivalDateTime);
  const now = new Date();
  return now > arrivalDate ? 'Concluida' : 'Activa';
});
